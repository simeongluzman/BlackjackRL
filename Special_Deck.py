...
class BlackJackEnv:
    ...
    def __init__(self, numDecks, isLow):
        # Removing 8 10s from each deck
        if isLow:
            self.deck = [2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 11] * 4 * numDecks
            self.rCount = -2 * 4 * numDecks  # Running count
        # Adding 8 10s to each deck
        else:
            self.deck = [2, 3, 4, 5, 6, 7, 8, 9, 10,
                         10, 10, 10, 10, 10, 11] * 4 * numDecks
            self.rCount = 2*4 * numDecks  # Running count
    ...
    def shuffle(self):
        ...
        self.discard = []
        if self.isLow:
            self.rCount = -2 * 4 * self.numDecks  # Running count
        else:
            self.rCount = 2 * 4 * self.numDecks  # Running count
